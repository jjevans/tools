...
#collection0 = "variant_queue_2222"
collection1 = "sanger_primers_v2"
...
#	st_rec = m_collect0.find_one({"seqevents.seqid": record["batch"], "_id": record["gpp"]})
	...
	primer_rec = m_collect1.find_one({"_id": int(record["primer"])})
	name = str(primer_rec[unicode("name")])
#	print str(record["primer"])
#	print str(name)
#	print str(record["_id"]) + "\t" + record["primer"] + "\t" + str(primer_rec["_id"])# + "\t" + primer_rec["name"]

	print str(record["_id"]) + "\t" + str(record["primer"]) + "\t" + name
#	m_collect0.update_one({"_id": record["_id"]}, {"$set": {"primer": name}})
