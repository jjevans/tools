...
die "usage: compare_rows.pl\n\ntakes as stdin from script common_cols.pl and finds ids (1st col each file) that are the same and eliminate any cols with same value with like header col names\n" unless @ARGV == 2;
...
for(my $i=0;$i<@colnames1;$i++){
	...
	if($colnum != -1){# column found in tbl2
		...
		my $diffrows = &eval_rows(\@colvals1,\@colvals2);
